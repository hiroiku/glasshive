...
import {
  ...
  type GithubIssueDiscussionEntry,
...
async function collect(walk: AsyncGenerator<GithubIssueDiscussionChunk, void, void>) {
  let head: GithubIssueDiscussionChunk & { kind: 'head' } = {
    kind: 'head',
    head: absent('empty'),
  };
  ...
  return { kind: opened ? head.head.kind : 'missing', entries, truncated };
}
...
describe('GitHub の課題 1 件のやり取り', () => {
  ...
  it('誰も何も言っていない課題を、読めなかったことにしない', async () => {
    ...
    const result = await collect(
      useCaseWith(tracker).stream({ projectPath: '/work/glasshive', number: 1 }),
    );

    expect(result.kind, '誰も書いていないのは、こちらが観測できた事実である').toBe('observed');
  ...
  it('その番号が応答に無ければ、無かったと言う', async () => {
    ...
    const result = await collect(
      useCaseWith(tracker).stream({ projectPath: '/work/glasshive', number: 99999 }),
    );

    expect(result.kind, '`gh` は答えている。その答えに無かったのは、観測できた事実である').toBe(
      'absent',
    );
  });
  ...
  it('尋ねられなければ、観測できなかったと言う', async () => {
    ...
    expect(result.kind, '空の並びにすると、誰も何も言っていない課題に見える').toBe('unobservable');
  });
  ...
  it('GitHub のリポジトリでなければ、尋ねない', async () => {
    ...
    const result = await collect(
      useCaseWith(tracker, gitWithoutRemote()).stream({
    ...
    expect(result.kind, 'remote を持たないのは「やり取りが読めなかった」ではない').toBe('absent');
  });
  ...
  it('次のページの在りかを渡して、続きを読む', async () => {
    ...
    expect(result.truncated, '最後まで読めたのに切れたと言わない').toBe(false);
  });
  ...
  it('ページ数の上限に当たったことを言う', async () => {
    ...
    expect(result.truncated, '黙って切ると、上限より後ろの発言が「無かった」ことになる').toBe(true);
  });
  ...
  it('途中で尋ねられなくなっても、観えたぶんは捨てない', async () => {
    ...
    expect(result.entries, '観えた 1 件を捨てると、やり取りが空になる').toHaveLength(1);
  ...
  it('途中のページが読めなくなっても、観えたぶんは捨てない', async () => {
    ...
    expect(result.kind, '1 ページ目は観えている').toBe('observed');
    expect(result.entries).toHaveLength(1);
...
describe('やり取りで名指された人の顔', () => {
  ...
  it('読めた人を、顔を引ける先として覚えさせる', async () => {
    ...
    await collect(
      useCaseWith(tracker, undefined, avatars).stream({
