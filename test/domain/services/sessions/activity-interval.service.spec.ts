...
describe('テキストから時刻を拾う', () => {
  it('入れ子の中の時刻も、現れた順のまま拾う', () => {
    ...
    expect(
      ...
      'パースせず、テキストをそのまま走査しているので、外側の 1 つだけにはならない',
  ...
  it('先頭が途中で切れていても、残りは拾える', () => {
    // `transcript` の末尾だけを切り出して渡すので、1 行目はたいてい途中から始まる
    const cutHead = [
      `estamp":"${at(0)}","type":"user"}`,
      `{"type":"assistant","timestamp":"${at(60_000)}"}`,
    ].join('\n');
    expect(
      extractTimestampsMs(cutHead),
      'キーの文字列が欠けたものを時刻と見なしてはいけない',
    ).toEqual([BASE + 60_000]);

    // 書いている途中の行は閉じ引用符が無い。表記が揃うまで時刻とは見なさない
...
describe('近い時刻を繋いで稼働区間にする', () => {
  it('幅ちょうどの無音は同じ区間、それを超えると別の区間', () => {
    expect(clusterIntervals([BASE, BASE + 120_000]), '境目は「以下」で切る').toEqual([
      { fromMs: BASE, toMs: BASE + 120_000 },
    ]);
    expect(clusterIntervals([BASE, BASE + 121_000]), '幅を超えた無音は区間を分ける').toEqual([
  ...
  it('順不同で渡しても同じ結果になる', () => {
    ...
    expect(shuffled, '渡された配列を書き換えると、呼んだ側の持ちものが壊れる').toEqual([
      BASE + 300_000,
      BASE,
      BASE + 360_000,
      BASE + 60_000,
  ...
  it('上限ちょうどならまとめ直さない', () => {
    ...
    expect(intervals.length, '上限を「超えたら」まとめる。ちょうどはまとめない').toBe(60);
  ...
  it('区間が多すぎるときはまとめ直すが、全体の始まりと終わりは変えない', () => {
  ...
  it('1 度広げて足りなければ、収まるまで広げ直す', () => {
    ...
    expect(
      clusterIntervals(timestamps, { max: 2 }),
      'まとめ直しは 1 度で終わるとは限らない',
    ).toEqual([{ fromMs: BASE, toMs: BASE + 1_500_000 }]);
  });

  it('まとめ直しでも、広げた幅ちょうどの無音は繋ぐ', () => {
  ...
  it('幅が広がらない渡し方をされても止まる', () => {
    // 幅を呼ぶ側から差し込める以上、いつまでもまとめ直しが終わらない経路を残せない
    expect(clusterIntervals([BASE, BASE + 1_000], { gapMs: 0, max: 0 })).toEqual([
      { fromMs: BASE, toMs: BASE },
      { fromMs: BASE + 1_000, toMs: BASE + 1_000 },
    ]);
    expect(clusterIntervals([BASE], { max: 0 }), '1 本より先はまとめられない').toEqual([
      { fromMs: BASE, toMs: BASE },
    ]);
  });
});

describe('拾ってまとめるところまで', () => {
  it('`transcript` のテキストから稼働区間を導く', () => {
    const text = [
  ...
  it('`transcript` の中で時刻が前後していても、数の順で区間にする', () => {
    // 拾うのは現れた順なので、繋ぐ前に並べ直さないと区間が崩れる
    const text = [
      `{"type":"user","timestamp":"${at(600_000)}"}`,
