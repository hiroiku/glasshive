...
describe('同じ実体の巣を 1 つに併せる', () => {
  it('解決済みの場所が同じなら、名前が違っても併さる', () => {
  ...
  it('代表は辞書順で最も小さい名前の持ち主', () => {
    ...
    expect(merged[0]?.path, '場所も代表のものを採る').toBe('/a/repo');
  ...
  it('場所がまったく分からないものは、名前が違えば併さらない', () => {
    const merged = mergeProjects([raw('-a', null, 10, [], null), raw('-b', null, 20, [], null)]);
    expect(merged, '場所が分からないもの同士を同じ実体と見なす根拠は無い').toHaveLength(2);
    expect(merged.map((m) => m.id)).toEqual(['-a', '-b']);
  });

  it('解決に失敗しても、生の場所が同じなら併さる', () => {
    const merged = mergeProjects([
    ...
    expect(
      merged,
      '解決の失敗は「揺れを均せなかった」だけで、「場所が分からない」ではない',
    ).toHaveLength(1);
    expect(merged[0]?.slugs).toEqual(['-a', '-b']);
  });

  it('解決に失敗したものも、解決済みの同じ場所と併さる', () => {
    const merged = mergeProjects([
    ...
    expect(merged[0]?.canonicalPath, '代表が解決できていなくても、組を作った場所は残る').toBe(
      '/Users/x/repo',
    );
  });

  it('帰属に使う場所は、代表が入れ替わっても動かない', () => {
    // 代表は名前の辞書順で決まるので、`-S` の側が代表になり path はそちらの字面になる。
    // ここで帰属まで代表の path で測ると、OS が教える場所と噛み合わなくなる。
    const merged = mergeProjects([
      raw('-Users-x-repo', '/Users/x/repo', 10, ['a'], '/Users/x/repo'),
      raw('-System-Volumes-Data-Users-x-repo', '/Users/x/repo', 20, ['b'], '/System/Volumes/x'),
    ]);
    expect(merged[0]?.path, '見せる字面は代表のもの').toBe('/System/Volumes/x');
    expect(merged[0]?.canonicalPath, '測るための場所は組のもの').toBe('/Users/x/repo');
  });

  it('生の場所が違えば、解決できていなくても併さらない', () => {
    const merged = mergeProjects([
  ...
  it('直近の動きは、組の中で最も新しいものを採る', () => {
    ...
    expect(merged[0]?.latestActivityMs, '併せた巣は、どれか 1 つでも動いていれば動いている').toBe(
      30,
    );
  });
  ...
  it('名前と場所は鍵の空間を分けて測る', () => {
    ...
    expect(merged, '字がたまたま同じでも、名前と場所は別のものである').toHaveLength(2);
  });
