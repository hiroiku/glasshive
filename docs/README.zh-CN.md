...
磁盘上的会话记录，把你在看的项目 —— 它们的会话与子智能体、每一个此刻正在做什么、它们的 issue，
以及它们当前活跃的 git 分支 —— 放到同一块屏幕上。可以把它想成智能体会话的 `htop`，只是没有 kill 键：
...
你在看的项目。等着你回应的排在前面，然后是仍在运行的。可以按名称、状态或时间范围筛选，也可以
调整标签栏的顺序。一开始是空的 —— 在仓库里运行 `glasshive`，那个仓库从此就被记录下来；或者从表格
上方「找到但没在看的目录」里挑一个加进来。
...
- **它唯一会写的文件是它自己的。** `~/.config/glasshive/preferences.json` 保存你在看的目录和视图偏好。
...
运行 `glasshive --help` 查看完整列表。

**指定一个目录，就是决定开始看它。** `glasshive .` 会记录并打开这个仓库；不带路径的 `glasshive`
在 git 仓库里做同样的事，不在仓库里则落到 Overview。给出的路径会解析到它所属的仓库，所以子目录或
worktree 都会到同一个地方。
