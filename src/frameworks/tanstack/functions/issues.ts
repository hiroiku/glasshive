...
/* 課題をブラウザーへ渡す server function。
   ...
   **この名前を `.server.ts` にしてはいけない。** 呼ぶのはブラウザー側なので、
   ...
   何ページにもなるものは、読めたページから順に渡す。**`async function*` を返すだけでよい** ——
   同期的に書き切れない値は、フレーム化した本文へ自動で切り替わる。応答のストリームそれ自体が
   この呼び出しとの結び付きなので、相関の id も、宛先を選ぶ仕掛けも要らない。 */
...
/* 開いた 1 件の本文。**一覧とは別に叩く。** 一覧に本文を混ぜると、100 件ぶんを運ぶことになる。
   ...
   ここだけページを歩かない。本文は 1 件につき 1 つで、届いたら全部である。 */
