...
/* 行の左の余白に、依存の弧を引く。
   ...
   記録の画面の枝と同じ文法で描く — 角は丸く、端は丸く、依存する先にだけ矢じり。
   2 つの画面で線の意味が同じなら、片方を読めればもう片方も読める。

   **この svg はレイアウトの外に置いてある。** 中に入れると、置換要素の固有の高さ 150px が
   行の高さに混ざって行が伸びる。列の幅は隣の空きが確保している。 */

/** 1 行の中での線の高さ。1 行運用での文字の中心 */
const ROW_MID = 14;
...
export function EdgeGutter({
  ...
  for (const edge of edges) {
    /* 弧そのものを鍵にする。**並びの中の位置ではない** — 行が並べ替わると位置は動くが、
       どの課題からどの課題への、どの筋の弧かは動かない。 */
    const key = `${edge.a}:${edge.b}:${edge.type}:${edge.lane}`;
    const x = laneX(edge.lane);
    const color = edgeColorOf(edge.type);
    /* 端が同じ行に重なることがある。終わり(矢じり)を上へ、始まり(丸)を下へずらして、
