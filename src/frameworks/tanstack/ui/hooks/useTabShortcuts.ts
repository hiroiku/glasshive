...
/* タブ行を鍵盤から扱う。位置で選び、位置を入れ替える。

   タブは位置で覚えて選ぶものなので、鍵盤に載せるならその位置がそのまま番号になる。
   1 が一覧、2 から先が留めたものである。

   **字を書いている手からは奪わない。** macOS では ⌘⇧← が「行の頭まで選ぶ」なので、
   探しの窓に居るあいだに奪うと、選ぼうとした人のタブが黙って動く。 */

/** 位置で選べる数。行の頭から数えて、これより先は番号を振らない */
export const MAX_SLOTS = 9;

/* ⌘ に当たる鍵。両方を受けると、Ctrl+数字がブラウザーのタブ切替と重なる盤で二重に効く。 */
...
export interface TabShortcutsProps {
  /** タブ行に出ている巣の id。行の並びそのもの */
  readonly visible: readonly string[];
  /** 留めた印の並び。動かす先はこちらの位置で数える */
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (!event.shiftKey && event.key >= '1' && event.key <= '9') {
        ...
        // 席が空いていれば何もしない。何もしないと分かるほうが、隣へ飛ぶより良い
        if (slot === 0) {
      ...
      // 押しっぱなしの連射で動かさない。1 回ごとに覚え書きへ置きに行く
      if (event.repeat) return;

      const at = visible.indexOf(current);
      if (at < 0) return;
      /* 隣は**行に出ている**ほうで数え、置く先は留めた印の位置で言う。
         留めた印には観測から消えたものも残っているので、印の上で 1 つ動かしても
         行の上では何も動かないことがある。 */
