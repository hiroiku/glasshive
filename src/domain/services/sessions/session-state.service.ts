/* セッションが今どの様子にあるか、そして誰を待っているかを決める。

   決め手は二つだけ。**最後に書かれてからどれだけ経ったか**と、**そのプロジェクトに
   生きているプロセスがいくつあるか**である。どのプロセスがどのセッションのものかは
...
/* トークンを数えに行く価値があるほど新しいか。
   ...
   **境界の向きが稼働の判定と違う。** ちょうど窓の幅に当たるものは外である。
   稼働の判定に使う `isWithinThreshold` へ `TOKEN_AGE_MS` を渡すと、7 日ちょうどの正本の
   扱いが裏返る。取り違えないよう、別の名前で置いてある。 */
...
/* 何を待っているかを見分ける。
   ...
   子が動いていて自分が止まっているなら、待っている相手は子である。自分も動いているなら
   まだ自分の番なので、誰も待っていない。子が一つも動いていなくて、末尾が自分の番の
   終わりを表しているときだけ、人の入力待ちと見る。

   **待ちを切るのは `ended` かどうかだけである。** 答えを返した直後のセッションは
...
function deriveAwaiting(
  ...
  // 下の二つは subsActive の真偽で排他なので、順は答えを変えない。旧の並びのまま置く
